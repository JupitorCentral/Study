...
spring 에서 Spring MVC 의 기본 HTTP exception 을 핸들링하는 추상 인터페이스이기 떄문이다.
먼저 익셉션이 발생하면ResponseEntityExceptionHandler 으로 넘어간다.

이 곳에는 자주 발생하는 Spring MVC 자주 발생하는 HTTP  Exception 들에 대한 핸들러가 정의되어 있다.
내가 만든 CustomException 은 여기에 해당되지 않기때문에 
ResponseEntityExceptionHandler 가 해당 Error 를 Throw 하고, 

```java
public final ResponseEntity<Object> handleExceptio
...
else {  
    // Unknown exception, typically a wrapper with a common MVC exception as cause  
    // (since @ExceptionHandler type declarations also match nested causes):    // We only deal with top-level MVC exceptions here, so let's rethrow the given    // exception for further processing through the HandlerExceptionResolver chain.   
     throw ex;  
}
```


그 다음에 내가 @ExceptionHandler 로 만든 처리 메서드가 실행되는 것이다.
...
근데 알고보니 ResponseEntityExceptionHandler 가 필요 없었다.
ResponseEntityExceptionHandler 라는 추상 클래스를 가지고 무언가 할 필요가 없었다.
```java
@RestControllerAdvice  
...
다른 에러가 발생했을떄 테스트를 해보진 않았지만 적어도 내 코드에서 발생한
Custom Exception 에 대해서는 ProblemDetail 형식으로 올바르게 리턴이 되었다.

하지만 ResponseEntityExceptionHandler 를 상속받으면
위에 설명했던 것처럼 Spring MVC 에서 Http 관련 자주 발생하는 익셉션에 대해서
SPring 이 제공하는 기본 핸들러를 내 프로젝트에 적용할 수 있기때문에
기본 템플릿을 적용하는 효과를 누릴 수 있다.
